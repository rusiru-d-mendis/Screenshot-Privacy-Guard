...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col p-4 md:p-8 font-sans">
      <header className="text-center mb-8">
        ...
        <p className="text-slate-600 mt-2 text-lg">Protect sensitive information with blur, pixelation, and powerful drawing tools.</p>
      </header>
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-3 gap-8">
        <aside className="lg:col-span-1 bg-white rounded-xl p-6 flex flex-col border border-slate-200 shadow-lg">
          <h2 className="text-2xl font-semibold mb-6 border-b border-slate-200 pb-4 text-slate-800">Controls</h2>
          <div className="flex flex-col space-y-4">
            ...
            <button onClick={handleAutoDetect} disabled={!imageFile || isDetecting} className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed text-white font-bold py-3 px-4 rounded-lg transition-colors duration-200 flex items-center justify-center gap-2">
            ...
            <div className="bg-slate-100 p-3 rounded-lg space-y-4 border border-slate-200">
              <div>
                  <h3 className="text-lg font-semibold mb-2 text-slate-700">Tool</h3>
                  <div className="flex bg-slate-200 rounded-lg p-1 space-x-1">
                      {[{ tool: 'pointer', icon: <PointerIcon /> }, { tool: 'rectangle', icon: <RectangleIcon /> }, { tool: 'ellipse', icon: <CircleIcon /> }, { tool: 'freehand', icon: <PencilIcon /> }].map(({ tool, icon }) => (
                          <button key={tool} onClick={() => setDrawingTool(tool as DrawingTool)} className={`w-1/4 rounded-md py-2 text-sm font-medium transition-colors flex justify-center items-center ${drawingTool === tool ? 'bg-blue-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-300/70'}`}>{icon}</button>
                      ))}
                  </div>
              </div>
              <div>
                <h3 className="text-lg font-semibold mb-2 text-slate-700">Effect Style</h3>
                <div className="flex bg-slate-200 rounded-lg p-1">
                  <button onClick={() => setEffectType('blur')} className={`w-1/2 rounded-md py-2 text-sm font-medium transition-colors ${effectType === 'blur' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-300/70'}`}>Blur</button>
                  <button onClick={() => setEffectType('pixelate')} className={`w-1/2 rounded-md py-2 text-sm font-medium transition-colors ${effectType === 'pixelate' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-300/70'}`}>Pixelate</button>
                </div>
                <div className="mt-3">
                  {effectType === 'blur' ? (
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <button onClick={handleReset} disabled={!imageFile || regions.length === 0} className="w-full bg-red-500 hover:bg-red-600 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed text-white font-bold py-3 px-4 rounded-lg transition-colors duration-200 flex items-center justify-center gap-2"><TrashIcon />Reset</button>
                <button onClick={handleDownload} disabled={!imageFile || regions.length === 0} className="w-full bg-emerald-500 hover:bg-emerald-600 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed text-white font-bold py-3 px-4 rounded-lg transition-colors duration-200 flex items-center justify-center gap-2"><DownloadIcon />Download</button>
            </div>
            {error && <p className="text-red-400 text-center">{error}</p>}
          </div>
          <div className="mt-auto pt-6 text-slate-500 text-sm">
            <h3 className="font-semibold text-lg text-slate-800 mb-2">How to use:</h3>
        ...
        <section className="lg:col-span-2 bg-white rounded-xl p-4 flex items-center justify-center border border-slate-200 shadow-lg min-h-[400px] lg:min-h-0">
          <div ref={imageContainerRef} className={`w-full h-full flex items-center justify-center overflow-hidden rounded-lg ${drawingTool === 'pointer' ? 'cursor-default' : 'cursor-crosshair'}`} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
            {!imageUrl ? (
              <div className="text-center text-slate-400"><UploadIcon className="w-16 h-16 mx-auto mb-4" /><p className="text-xl">Your image will appear here</p></div>
            ) : (
